...
class MMPIQuestionsViewController: UIViewController, UITabBarControllerDelegate {
    ...
    var bluebird: bluebirdModel? {
        get {
            return bluebirdModel(userPhoto: userPhoto ?? (UIImage(named: "camera")?.pngData())!,
                                firstName: firstName ?? "",
                                lastName: lastName ?? "",
                                ...
                                lastModified: Date(),
                                isMale: isMale ?? true,
                                mmpiStorage: mmpiStorage,
                                mmpiTestDates: mmpiTestDates,
                                tScoresStorage: tScoresStorage,
                                tsiStorage: tsiStorage,
                                tsiTestDates: tsiTestDates,
                                tsiScoresStorage: tsiScoresStorage)
        }
        set {
            countLabel.text = "" // clearing previous value
            if newValue != nil {
                userPhoto = newValue!.userPhoto
                firstName = newValue!.firstName
                lastName = newValue!.lastName
                patronimicName = newValue!.patronimicName
                birthDay = newValue!.birthDay
                birthMonth = newValue!.birthMonth
                birthYear = newValue!.birthYear
                notes = newValue!.notes
                lastModified = newValue!.lastModified
                isMale = newValue!.isMale
                mmpiStorage = newValue!.mmpiStorage
                mmpiTestDates = newValue!.mmpiTestDates
                tScoresStorage = newValue!.tScoresStorage
                tsiStorage = newValue!.tsiStorage
                tsiTestDates = newValue!.tsiTestDates
                tsiScoresStorage = newValue!.tsiScoresStorage
            }
        }
    }
    
    private var userPhoto: Data?
    private var firstName: String?
    private var lastName: String?
    private var patronimicName: String?
    ...
    private var mmpiStorage = [[Int: Int]]()
    private var mmpiTestDates = [Date]()
    private var tScoresStorage = [[String: Int]]()
    private var tsiStorage = [[Int: String]]()
    private var tsiTestDates = [Date]()
    private var tsiScoresStorage = [[String: Int]]()
    
    var currentSessionNumber: Int = 0
        ...
        didSet {
            if previousButton != nil {
                if questionNumber == 1 {
                    ...
                    questionLabel.text = "Пожалуйста перейдите на вкладку \"История\" для получения резульатов"
                    documentChanged()
                } else {
    ...
    private var minimumQuestion = 1
    private var maximumQuestion = 378
    
    private var currentAnswersArray = [Int: Int]() {
        didSet {
            ...
            if document?.bluebird != nil {
                ...
                if currentSessionNumber >= mmpiStorage.count || mmpiStorage.count == 0 {
                    lastModified = Date()
                    mmpiTestDates.append(Date())
                    mmpiStorage.append(currentAnswersArray)
                } else {
                    lastModified = Date()
                    mmpiTestDates[currentSessionNumber] = Date()
                    mmpiStorage[currentSessionNumber] = currentAnswersArray
                }
            }
    ...
    let progress = Progress(totalUnitCount: 377)
    
    // MARK: - Outlets
    
    @IBOutlet weak var genderLabel: UILabel! {
        didSet {
            if let isMaleFromDoc = document?.bluebird?.isMale {
                if isMaleFromDoc {
                    genderLabel.text = "Мужской пол"
                } else {
                    genderLabel.text = "Женский пол"
                }
            }
        }
    }
    ...
    @IBAction func previousButtonPressed(_ sender: UIButton) {
        if isValidQuestionNumber(number: questionNumber) {
            currentAnswersArray[questionNumber] = 0
            
            if questionNumber != minimumQuestion {
                questionNumber -= 1
                changeStatsOnScreen(for: questionNumber)
            }
        }
    }
    ...
    @IBAction func nextButtonPressed(_ sender: UIButton) {
        if isValidQuestionNumber(number: questionNumber) {
            ...
            if questionNumber != maximumQuestion {
                questionNumber += 1
                changeStatsOnScreen(for: questionNumber)
            }
        }
    ...
    @IBAction func yesButtonPressed(_ sender: UIButton) {
        if isValidQuestionNumber(number: questionNumber) {
            currentAnswersArray[questionNumber] = 1
    ...
    @IBAction func noButtonPressed(_ sender: Any) {
        if isValidQuestionNumber(number: questionNumber) {
            currentAnswersArray[questionNumber] = 2
    ...
    private func changeStatsOnScreen(for questionNum: Int) {
        countLabel.text = "Вопрос " + String(questionNum)
        questionLabel.text = questionsDictionary[questionNum]
        
        progress.completedUnitCount = Int64(questionNum)
        progressView.setProgress(Float(self.progress.fractionCompleted), animated: true)
        
        documentChanged()
    }
    
    private func isValidQuestionNumber(number: Int) -> Bool {
        if number >= minimumQuestion && number <= maximumQuestion {
            return true
        }
        return false
    }
    
    var isContinuingUnfinishedTest: Bool? {
        didSet {
            if !isContinuingUnfinishedTest! {
                if let numberOfEntries = document?.bluebird?.mmpiStorage.count {
                    currentSessionNumber = numberOfEntries
                } else {
                    currentSessionNumber = 0
                }
            } else {
                if let currentAns = document?.bluebird?.mmpiStorage[currentSessionNumber] {
                    print("adding array")
                    currentAnswersArray = currentAns
                }
            }
        }
    }
    ...
    override func viewDidLoad() {
        ...
        countLabel.text = "Вопрос " + String(questionNumber)
        questionLabel.text = questionsDictionary[questionNumber]
        
        previousButton.isEnabled = false
        
        progress.completedUnitCount = Int64(questionNumber)
        progressView.setProgress(Float(self.progress.fractionCompleted), animated: true)
    ...
    */
    ...
    // MARK: - Questions
    /* taken from http://docpsy.ru/testy/diagnostika-ps-sostoyanij-i-svojstv-lichnosti/3946-oprosnik-mmpi.html */
    private let questionsDictionary = [1: "Вам понравилась бы работа медсестры.",
                                            ...
                                            378: "Тест завершен. Вы можете посмотреть реузльтаты в секции \"Результаты теста\""]
}
